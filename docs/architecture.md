...
| --- | --- |
| Presentation and interaction | `app.py` and `frontend/` |
...
These boundaries keep Streamlit rerun and session-state concerns out of the
domain, retrieval, provider, and persistence layers.
...
used. The PostgreSQL memory foundation is append-only; the null store supplies
no durable history. Export sanitization is implemented at the memory boundary.
...
- PostgreSQL memory is an optional append-only foundation, not complete note
  CRUD or a production-grade multi-user memory system.
