...
def _learning_path_map_steps(schema: LearningDirectionSchema) -> list[tuple[str, str]]:
...
def build_full_learning_plan_markdown(
    *,
    ...
    reason = _markdown_text(
        _noise_to_signal_reason(decision) or decision.get("recommendation"),
    ...
    note_tags = _markdown_list(note.get("tags") if isinstance(note.get("tags"), list) else [])
